...
public function getUnitReportByMonth(Request $req)
            ...
            foreach($unit_id as $unitId){
                ...
                if($users !== null){
                    ...
                    foreach($users as $user){
                        ...
                        if(($user['user']['leave_date'] !== null && strtotime($user['user']['leave_date']) < strtotime($end)) || ($user['user']['join_date'] !== null && strtotime($user['user']['join_date']) > strtotime($end)) || ($user['user']['user_role'] ==1) || ($user['user']['user_role'] ==2) ){
                            continue;
                        }
    ...
    public function unit_sheets_get_users(Request $req)
    {
        ...
        foreach ($users as $userDataKey => $userdata) {
            ...
            foreach ($months as $key => $month) {
                ...
                $should_be_at = ($target / $this->getTotalWorkingDays($month, intval($years[$key]))) *  $this->getWorkdays(intval($years[$key]) . "-" . $month . "-01", $date2);
    ...
    public function specificleads(Request $request, $leads)
    {
        ...
        if ($leads) {
            ...
            $leads->date = $leads->created_at->format('jS M Y');
            $l = $leads->toArray();
...
$unitid = array_column($unitid, 'unit_id');
            ...
            $units = Units::whereIn('id',$unitid)->get()->toArray();
        }
    ...
    public function BrandsAddEdit(Request $request, Brands $brand)
    {
        ...
        $check = Brands::where('name', $request->name)->first();
        ...
        if (!$check) {
            ...
            return response()->json(['status' => false, 'msg' => 'It is Already Exist!']);
        }
    }
    ...
    public function leadslist(Request $req)
    {
        ...
        if ((auth()->user()->role->id == 4 && auth()->user()->permission == 3) || (auth()->user()->role->id == 2 || auth()->user()->permission == 3)) {
    ...
    public function getUnitTargets($unitId)
    {
        ...
        $unit = Units::select('id', 'name')->with('MonthTarget')->where('id', $unit_target->id)->where('status', 1)->first();
    ...
    public function unit_user($unit_id = 2)
    {
        ...
        if (auth()->user()->role->id == 4 && auth()->user()->permission !== null && (auth()->user()->permission == 2 || auth()->user()->permission == 3)) {
            $data = User::with(
                ...
                ->where('user_role', '!=', '1')->where('status', 1)->where('id', auth()->user()->id)->get()->toArray();
        } else {
        ...
        $user_data = [];
        foreach ($data as $value) {
            if($value['user_role']==2 || $value['user_role']==3)
            {
                continue;
            }
            $grosssum = 0;
